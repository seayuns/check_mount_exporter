...
var (
	...
	configCheckIO            = kingpin.Flag("config.check-io", "Enable read/write IO probes for mounted paths").Default("false").Bool()
	configCheckIOTimeout     = kingpin.Flag("config.check-io-timeout", "Timeout for each IO probe").Default("30s").Duration()
...
func mountpointPath(mountpoint string) string {
	if *rootfsPath == "/" {
		return mountpoint
	}
	return filepath.Join(*rootfsPath, strings.TrimPrefix(mountpoint, "/"))
}

func defaultProbeMountpointIO(mountPath string, rw bool) bool {
	...
	f, err := os.CreateTemp(mountPath, ".check_mount_exporter_io_*")
	if err != nil {
		return false
	}
	name := f.Name()
	payload := []byte("check_mount_exporter\n")
	...
	return string(readBack) == string(payload)
}

func probeMountpointIOWithContext(ctx context.Context, mountPath string, rw bool) bool {
	result := make(chan bool, 1)
	go func() {
		result <- defaultProbeMountpointIO(mountPath, rw)
	}()
	select {
	case <-ctx.Done():
		return false
...
var probeMountpointIO = probeMountpointIOWithContext
...
func NewExporter(mountpoints []string, logger log.Logger) *Exporter {
	...
	return &Exporter{
		...
		ioStatus:                  prometheus.NewDesc("check_mount_io_status", "Mount point IO status, 1=accessible 0=failed", []string{"mountpoint", "rw"}, nil),
...
func (e *Exporter) collect() ([]CheckMountMetric, error) {
	...
	for _, m := range e.mountpoints {
		...
		if *configCheckIO {
			metric.hasIOCheck = true
			if mounted {
				mountPath := mountpointPath(m)
				probeCtx, cancel := context.WithTimeout(context.Background(), *configCheckIOTimeout)
				if probeMountpointIO(probeCtx, mountPath, rw) {
					metric.ioStatus = 1
				}
				cancel()
			}
...
func (e *Exporter) Collect(ch chan<- prometheus.Metric) {
	...
	for _, m := range metrics {
		...
		if *configCheckIO && m.hasIOCheck {
			ch <- prometheus.MustNewConstMetric(e.ioStatus, prometheus.GaugeValue, m.ioStatus, m.mountpoint, m.rw)
		}
	}
}
